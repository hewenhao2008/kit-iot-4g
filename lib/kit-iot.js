...
var KitIoT = function (options) {
  ...
  this.loopTime = options.loopTime || 30000;
  this.isConsole = options.isConsole || false;
  this.showLog = options.showLog || false;

  this.server = new kit.Server({ port: '4000', console: this.isConsole });
...
KitIoT.prototype.start = function () {
  ...
  var doUpdateDashboard = function () {
    ...
    if (self.isConsole) {
      if (self.showLog && self.isPrintable) {
        log(
          '*----------------------*\n' +
          '*      Dashboard       *\n' +
          '*----------------------*\n' +
  ...
  self.loop = setInterval(doSaveData, self.loopTime);
  self.loopDash = setInterval(doUpdateDashboard, self.loopTime);
};
...
KitIoT.prototype.saveData = function (data) {
  ...
  if (!apiKey) {
    self.io.sockets.emit('no-internetConnection', { msg: 'Service não encontrado' });
